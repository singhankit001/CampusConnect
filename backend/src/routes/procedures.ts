...
router.post('/enroll-student', async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const result = await prisma.$queryRawUnsafe<any[]>(
      `SELECT EnrollStudent($1, $2, $3, $4) AS "enrollmentId"`,
      studentId,
      courseId,
      semester,
      academicYear
...
router.post('/apply-internship', async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const result = await prisma.$queryRawUnsafe<any[]>(
      `SELECT ApplyForInternship($1, $2, $3, $4) AS "applicationId"`,
      studentId,
      internshipId,
      resumeUrl,
      coverLetter || ''
...
router.post('/register-event', async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const result = await prisma.$queryRawUnsafe<any[]>(
      `SELECT RegisterForEvent($1, $2) AS "registrationId"`,
      userId,
      eventId
    );
...
router.post('/submit-assignment', async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const result = await prisma.$queryRawUnsafe<any[]>(
      `SELECT SubmitAssignment($1, $2, $3) AS "submissionId"`,
      studentId,
      assignmentId,
      fileUrl
    );
...
router.get('/student-report/:studentId', async (req: AuthenticatedRequest, res: Response) => {
  const { studentId } = req.params;

  try {
    // Call: SELECT GenerateStudentReport(studentId);
    const result = await prisma.$queryRawUnsafe<any[]>(
      `SELECT GenerateStudentReport($1) AS report`,
      studentId
    );
