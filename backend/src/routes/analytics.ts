...
router.get('/student-dashboard/:studentId', async (req: AuthenticatedRequest, res: Response) => {
  const { studentId } = req.params;
  try {
    // 1. Fetch Student general details using the View
    const performanceView = await prisma.$queryRawUnsafe<any[]>(
      `SELECT * FROM "StudentPerformanceView" WHERE student_id = $1`,
      studentId
    );
    ...
    const pendingAssignments = await prisma.$queryRawUnsafe<any[]>(
...
router.get('/faculty-dashboard/:facultyId', async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    let coursesAnalytics: any[] = [];
    if (courseIds.length > 0) {
      const placeholders = courseIds.map((_, i) => `$${i + 1}`).join(',');
      coursesAnalytics = await prisma.$queryRawUnsafe<any[]>(
        `SELECT * FROM "CourseAnalyticsView" WHERE course_id IN (${placeholders})`,
        ...courseIds
      );
    }
...
router.get('/recruiter-dashboard/:companyId', async (req: AuthenticatedRequest, res: Response) => {
  const { companyId } = req.params;
  try {
    // 1. Get company placements analytics from View
    const statsView = await prisma.$queryRawUnsafe<any[]>(
      `SELECT * FROM "PlacementStatisticsView" WHERE company_id = $1`,
      companyId
    );
    ...
    const funnel = await prisma.$queryRawUnsafe<any[]>(
...
router.get('/admin-dashboard', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const deptPlacements = await prisma.$queryRawUnsafe<any[]>(
...
export const ANALYTICAL_QUERIES = [
  {
    id: 1,
...
router.get('/queries', (req: AuthenticatedRequest, res: Response) => {
  return res.json(ANALYTICAL_QUERIES.map(q => ({ id: q.id, title: q.title, purpose: q.purpose, sql: q.sql, explanation: q.explanation })));
});

// POST to execute a specific query by ID
router.post('/queries/:id/run', async (req: AuthenticatedRequest, res: Response) => {
  const queryId = parseInt(req.params.id);
  const qDef = ANALYTICAL_QUERIES.find(q => q.id === queryId);
  ...
  try {
    const results = await prisma.$queryRawUnsafe<any[]>(qDef.sql);
    return res.json({
      id: qDef.id,
      title: qDef.title,
